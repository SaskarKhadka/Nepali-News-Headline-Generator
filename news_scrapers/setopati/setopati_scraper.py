...
SETOPATI_WEBSITES = {
                    #   "राजनीति": "https://www.setopati.com/politics",
...
for key, value in SETOPATI_WEBSITES.items():
    driver.get(value)
    time.sleep(2)
    ...
    for page in range(150, 500):
        ...
        category_news = driver.find_elements(By.CSS_SELECTOR, 'section.section.special-news.cat-list div.row.bishesh.news-cat-list')
        news_list += category_news
        
        news_cover_links = []
        news_titles = []
        for news_cover in news_list:
            news_link = news_cover.find_element(By.CSS_SELECTOR, 'div.items a')
        ...
        with codecs.open('setopati_news_links.csv', 'a', 'utf-8') as file:
            file.write("\n".join(news_cover_links))
            file.write("\n")
            
        with codecs.open('setopati_news_titles.csv', 'a', 'utf-8') as file:
                file.write("\n".join(news_titles))
                file.write("\n")
