...
for key, value in UJYAALOONLINE_WEBSITES.items():
    driver.get(value)
    time.sleep(2)
    ...
    for page in range(1, 78):
        ...
        category_news = driver.find_elements(By.CSS_SELECTOR, 'div.grid-posts div.col-md-4.padding-10 div.grid-post')
        news_list += category_news
        
        news_cover_links = []
        news_titles = []
        for news_cover in news_list:
            news_link = news_cover.find_element(By.CSS_SELECTOR, 'div h3 a.text-dark')
            news_titles.append(news_link.text)
            news_cover_links.append(news_link.get_attribute('href'))
        
        with codecs.open('ujyaalo_news_links.csv', 'a', 'utf-8') as file:
            file.write("\n".join(news_cover_links))
            file.write("\n")
            
        with codecs.open('ujyaalo_news_titles.csv', 'a', 'utf-8') as file:
                file.write("\n".join(news_titles))
                file.write("\n")
