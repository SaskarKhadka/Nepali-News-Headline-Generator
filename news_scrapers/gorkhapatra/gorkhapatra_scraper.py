...
for key, value in GORKHAPATRA_WEBSITES.items():
    ...
    for page in range(1, 120):
        ...
        for news_cover in news_list:
            news_link = news_cover.find_element(By.CSS_SELECTOR, "h2.item-title a")
            news_titles.append(news_link.text)
            news_cover_links.append(news_link.get_attribute("href"))

        with codecs.open("gorkhapatra_news_links.csv", "a", "utf-8") as file:
            file.write("\n".join(news_cover_links))
            file.write("\n")

        with codecs.open("gorkhapatra_news_titles.csv", "a", "utf-8") as file:
            file.write("\n".join(news_titles))
            file.write("\n")
